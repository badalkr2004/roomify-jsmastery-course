...
import {
    createHostingSlug, fetchBlobFromUrl, getHostedUrl, getImageExtension,
    HOSTING_CONFIG_KEY,
    imageUrlToPngBlob,
    isHostedUrl,
} from "./utils";
...
export const uploadImageToHosting = async ({
  ...
  try {
    ...
    if(!resolved) return null
      ...
      return hostedUrl ? ({ url: hostedUrl } as HostedAsset) : null;
  } catch (e) {
    console.warn(`Could not upload image to hosting: ${e}`);
    return null;
  }
};
